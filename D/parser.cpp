...
void preOrderTraverse(treeNode *node, int depth) {
    ...
    // Print the attributes of the current node
    std::cout << "Node String: " << node->nodeString << std::endl;
    std::cout << "Type: " << node->type << std::endl;

    // Recursively traverse the child nodes with increased depth
    preOrderTraverse(node->childNode, depth + 1);

    // Recursively traverse the sibling nodes with the same depth
